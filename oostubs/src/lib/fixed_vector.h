...
namespace oostubs
{
	namespace utils
	{
			...
			public:
				...
				template<typename TT>
					void emplace_back(TT&& v)
						{ new(end()) value_type(std::forward<TT>(v)); ++mSize; }
		...
		void FixedVector<N, T>::insert(const_iterator i, TT&& v)
		{
			...
			for(; i != j ; --j)
			{
				new(j) value_type(std::move(j[-1]));
				j[-1].~value_type();
			}

			new(j) value_type(std::forward<TT>(v));
